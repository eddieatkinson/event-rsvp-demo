...
export default function Home() {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      setError(err?.message || "Something went wrong fetching events.");
    }
  };
  ...
  return (
    <main className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-50 flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-3xl space-y-6">
        {/* Events List Section */}
        <section className="rounded-xl border border-slate-800 bg-slate-900/70 px-4 py-4 sm:px-5 sm:py-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-indigo-500/10 border border-indigo-500/30 px-3 py-1.5 text-xs font-medium text-indigo-300 transition hover:bg-indigo-500/20 hover:border-indigo-500/50"
            >
            ...
            {events.length > 0 ? (
              events.map((e) => (
                <button
                  key={e._id}
                  onClick={() => setEvent(e)}
                  className={`w-full flex items-center justify-between gap-3 rounded-lg border px-3 py-2.5 text-left transition ${
                    event?._id === e._id
                      ? "border-indigo-500/50 bg-indigo-500/10 text-slate-50"
                      : "border-slate-800 bg-slate-900/50 text-slate-300 hover:border-slate-700 hover:bg-slate-900/80"
                  }`}
                >
                  ...
                  <div className="flex items-center gap-2">
                    {e.rsvps && e.rsvps.length > 0 && (
                      <span className="inline-flex items-center rounded-full bg-slate-800/80 px-2 py-0.5 text-[0.65rem] text-slate-400">
        ...
        {/* Main Event/RSVP Section */}
        <div className="rounded-2xl bg-slate-900/80 border border-slate-800 shadow-2xl shadow-slate-950/40 backdrop-blur px-6 py-6 sm:p-8 space-y-6">
          <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">
                Create a simple event, collect RSVPs, and preview everyone who
                is comingall in one place.
              </p>
            </div>
              ...
              {!hasEvent ? (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      className="mt-1 w-full min-w-0 rounded-lg border border-slate-700 bg-slate-900/70 px-3 py-2 text-sm text-slate-50 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                      style={{ maxWidth: "100%" }}
                      value={date}
